...
export const ApiConfigurator: React.FC<ApiConfiguratorProps> = ({ currentConfig, onSave }) => {
  ...
  const handleValidateAndSave = async () => {
    ...
    if (result.isValid) {
      ...
      // Still save, but as unvalidated
      onSave({ provider, apiKey, model, isValidated: false });
    }
  };

  return (
    <div className="p-6 bg-slate-800/50 rounded-2xl border border-slate-700 mb-8 shadow-lg transition-all duration-300 animate-fade-in">
      <h2 className="text-2xl font-bold mb-1 text-slate-100 flex items-center gap-3"><KeyIcon className="w-6 h-6 text-yellow-300" /> API Configuration</h2>
      {!currentConfig.isValidated && (
         <p className="text-sm text-yellow-400 mb-4">Please configure a valid API key to begin generating content.</p>
      )}

      <div className="space-y-4">
        {/* Provider Selection */}
        <div>
          <label htmlFor="provider" className="block text-sm font-medium mb-1 text-slate-300">AI Provider</label>
          <select
            id="provider"
            value={provider}
            onChange={(e) => handleProviderChange(e.target.value as AiProvider)}
            className="w-full bg-slate-800 border border-slate-600 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          >
        ...
        {isGemini ? (
             <div className="bg-slate-900/50 text-slate-300 p-3 rounded-lg text-sm flex items-center gap-3 border border-slate-700">
        ...
        <div>
            <label htmlFor="apiKey" className="block text-sm font-medium mb-1 text-slate-300">
                API Key {isGemini && '(Optional)'}
                {providerInfo && (
                    <a href={providerInfo.apiKeyUrl} target="_blank" rel="noopener noreferrer" className="ml-2 text-cyan-400 hover:underline text-xs">(Get Key)</a>
                )}
            </label>
           <input
            id="apiKey"
            ...
            className="w-full bg-slate-800 border border-slate-600 rounded-lg px-4 py-2 placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          />
        </div>

        {provider === AiProvider.OpenRouter && (
            <div>
                 <label htmlFor="model" className="block text-sm font-medium mb-1 text-slate-300">Model Name (from OpenRouter)</label>
                 <input
                    id="model"
                    type="text"
                    value={model}
        ...
        <div>
          <button
            ...
            className="w-full font-bold py-2.5 px-5 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-50 disabled:cursor-not-allowed"
          >
